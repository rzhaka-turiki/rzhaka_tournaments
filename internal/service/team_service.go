...
type TeamService interface {
	...
	Update(ctx context.Context, actorID uuid.UUID, team *model.Team) error
...
func (s *teamService) Update(ctx context.Context, actorID uuid.UUID, team *model.Team) error {
	return s.txManager.WithinTransaction(ctx, func(tx pgx.Tx) error {
		teamRepo := repository.NewTeamRepository(tx)
		snapshotRepo := repository.NewTeamSnapshotRepository(tx)
		isOwner, err := teamRepo.IsOwner(ctx, team.ID, actorID)
		if err != nil {
			return err
		}
		if !isOwner {
